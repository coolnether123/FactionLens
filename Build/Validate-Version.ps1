...
{
    throw 'Faction Lens validation must run through the Cascade executor.'
}
...
$result = Test-RwtPackage `
    ...
    -Version $Version `
    -ExpectedAssemblyName 'FactionLens.dll'
if (-not $result.Succeeded)
{
    throw "Faction Lens package validation failed for ${Version}: $($result.Code) $($result.Message)"
}
